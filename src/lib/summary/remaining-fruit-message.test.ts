...
import { STOCK_REPORT_TITLE } from "./stock-summary-message";
import { fetchRemainingFruitRows } from "./remaining-fruit-data";
import { UNIDENTIFIED_MARKET_SECTION } from "./remaining-fruit";
...
describe("buildRemainingFruitMessagesFromRows", () => {
  ...
  test("includes overall section before market details for multi-market data", () => {
    ...
    const joined = messages.join("\n\n");
    // The all-market view now leads with the shared StockSummary executive
    // block (categorized product totals) instead of the old flat overall list.
    expect(messages[0]).toContain(STOCK_REPORT_TITLE);
    expect(messages[0]).toContain("\uD83C\uDF49 \u0E1C\u0E25\u0E44\u0E21\u0E49");
    expect(messages[0]).toContain(`${WATERMELON} \u2014 35 \u0E25\u0E39\u0E01`);
    expect(joined).toContain("35 \u0E25\u0E39\u0E01");

    // Per-market detail is still preserved, after the executive block.
  ...
  test("separates confirmed and unidentified totals in all-market report", () => {
    ...
    expect(joined.indexOf(STOCK_REPORT_TITLE)).toBeGreaterThanOrEqual(0);
    expect(joined.indexOf(STOCK_REPORT_TITLE)).toBeLessThan(joined.indexOf(UNIDENTIFIED_MARKET_SECTION));
    assertWithinLineLimits(messages);
  });
  ...
  test("2026-06-23 all-market production shape splits safely for LINE", async () => {
    ...
    expect(overallText).toContain(STOCK_REPORT_TITLE);
