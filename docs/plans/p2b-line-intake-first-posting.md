...
9. Notification: optionally, if still in the original request/response
     cycle and a reply token is available, send a short ephemeral
     acknowledgement (e.g. "บันทึกเข้าสต๊อกแล้ว กำลังส่งสรุป") via the reply
     API — this is fire-and-forget UX only, creates no outbox row, marks
     nothing delivered, and its success or failure has no bearing on the
     steps below (§18.7). Then: parts = create_purchase_capture_notification_parts({
     ...
     outbox push worker, not a variant per timing (§18.6, §18.7).
```
