...
- A P2B persistence contract (`src/lib/purchase-receipts/`, migration `20260729084558_purchase_receipt_persistence.sql`) with full-replace drafts, an idempotent `confirm`, immutable confirmed receipts, and a correction-by-supersession model — CONFIRMED, and its schema/RPCs/grants were independently verified to exist in Production exactly as migrated.
- A P2C quantity ledger (`src/lib/inventory-ledger/`, migration `20260729172613_inventory_movement_ledger.sql`) with idempotent posting, append-only movement lines, a derived (never stored) balance view, and hard RPC-level refusal of unresolved product/unit identity — CONFIRMED, also verified present in Production.
...
### 2.2 P2B persistence — `src/lib/purchase-receipts/`, migration `20260729084558_purchase_receipt_persistence.sql`
...
- **`documentNamespace` + `documentKey` are the document's identity, not a per-delivery event id** (`types.ts:145-147`). The exact contract quoted from the migration itself (`20260729084558_purchase_receipt_persistence.sql:75-79`):
...
### 2.3 P2C ledger — `src/lib/inventory-ledger/`, migration `20260729172613_inventory_movement_ledger.sql`, `docs/p2c-inventory-movement-ledger.md`
...
### 2.6 Guided Operations identity/ownership rules — `docs/guided-operations-end-to-end.md`, migrations `20260729074617_guided_menu_identity_and_state.sql`, `20260730093019_atomic_guided_owner.sql`
...
Both the session `open` RPC and every subsequent mutating (or content-returning read) RPC must take `p_expected_generation` **and** `p_expected_source_type`/`p_expected_source_id`/`p_expected_sender_line_user_id` (mirroring `admit_physical_inventory_event`'s `p_expected_generation`, §2.4, extended with the explicit ownership triple) and re-verify all four values against the locked session row before mutating — a staff member cannot append to, close, cancel, or even read another sender's session because the RPC itself refuses an `ownership_mismatch`/`generation_conflict`, not because the webhook handler happens to route correctly, and not merely because `session_generation` is hard to guess. This mirrors 0052's own fail-closed behavior on a document-key reused under a different source binding (§2.2). **Concretely, in Slice A** (§16.4): `admit_purchase_capture_event`, `close_purchase_capture_open_event`, `cancel_purchase_capture_session`, and `get_purchase_capture_finalize_candidate` all take the ownership triple as explicit parameters and compare it to the locked (or, for the read, queried-under-identity) session row, in the same statement that checks `session_generation` — a caller that supplies a correct `session_id`/`session_generation` but the wrong `source_type`/`source_id`/`sender_line_user_id` is refused `ownership_mismatch` before any mutation, and, for the read, before any content is returned. `open_purchase_capture_session` has no "expected" ownership to check against, since it is the RPC that establishes ownership in the first place.
...
Sent after the session transitions `closing` → `awaiting_confirmation`, via the exact same claim-then-push sequence used everywhere else in this design (§18.4, §18.7), regardless of whether `finalize_purchase_capture_session` ran synchronously inside the webhook request, in `after()`, or from the cron sweep: `finalize_purchase_capture_session` commits the receipt, the session's `awaiting_confirmation` transition, and the first `preview_ready` part set together, in one transaction (§16.4) → optionally, if a reply token is still available, an ephemeral acknowledgement may be sent (e.g. "ปิดใบซื้อแล้ว กำลังสรุป") — fire-and-forget, no outbox row, no bearing on what follows (§18.7) → `claim_next_purchase_capture_notification_part` claims part 0 → `pushLineMessage()` delivers it using that part's `retry_key` → the part is marked delivered → the next part (if any) is claimed and pushed the same way, in order, until the whole preview has been delivered. The rendered text below is the content of that preview's part(s):
...
8. completeResult = complete_purchase_capture_posting({ sessionId,
     expectedGeneration, movementId: postResult.movementId,
     postedSuccessPayloadTexts: [renderedPostedSuccessText] }) — ONE RPC
     call, ONE transaction, the sole authoritative posting-completion gate
     (§16.4), replacing the earlier two-step "mark session posted, then
     separately create the notification" sequence whose gap between the
     two steps this closes:
   a. Lock the session row FOR UPDATE — the same lock this session has
      already been under continuously since step 3h; no new session can
      race in here.
   b. Re-check source_type/source_id/sender_line_user_id/session_generation
      match; refuse `ownership_mismatch`/`generation_mismatch` otherwise
      (defensive — this session has not changed identity since step 3, but
      the RPC re-verifies rather than trusting the caller regardless).
   c. Re-check status: `'confirming'` → proceed to (d); already `'posted'`
      with `session.movement_id = p_movement_id` → treat as an idempotent
      replay of this exact call (skip straight to (d)'s part-set
      idempotent-replay behavior, do not re-run (e)/(f)); already `'posted'`
      with a **different** `movement_id`, or any other status → refuse
      `invalid_state` — this is what makes "posted_success parts cannot
      exist for a movement not attached to that session" true by
      construction, since (d) below never runs against a mismatched
      movement.
   d. Create the complete `posted_success` part set — `notification_kind =
      'posted_success'`, `notification_version = p_movement_id`,
      `payload_texts = p_posted_success_payload_texts` — using the exact
      same atomic multipart-creation logic `create_purchase_capture_notification_parts`
      uses (§16.3, §18.3), inline in this same transaction, not a separate
      RPC call: same identity + byte-identical payload → idempotent replay
      of the existing rows; same identity + different payload →
      `notification_identity_conflict`, and this whole transaction rolls
      back — the session is **not** transitioned to `'posted'` on a
      payload conflict, it stays `'confirming'`, exactly as if this call
      had never been made.
   e. UPDATE the already-locked session row: `status = 'posted'`,
      `movement_id = p_movement_id`, write a lifecycle event.
   f. Commit. Steps (d)-(e) either both land or neither does — Postgres
      transaction atomicity, not application coordination, guarantees this.
   Any refusal (`ownership_mismatch`, `generation_mismatch`, `invalid_state`,
   `notification_identity_conflict`) leaves the session exactly as it was
   before this call — `'confirming'`, no `movement_id` recorded if it
   wasn't already, no partial `posted_success` rows — so a caller can
   always retry from step 8 itself using the same `postResult.movementId`
   it already has.
9. Notification delivery: optionally, if still in the original
     request/response cycle and a reply token is available, send a short
     ephemeral acknowledgement (e.g. "บันทึกเข้าสต๊อกแล้ว กำลังส่งสรุป") via
     the reply API — **only after step 8's transaction has committed**,
     never before; this is fire-and-forget UX only, creates no outbox row,
     marks nothing delivered, and its success or failure has no bearing on
     what follows (§18.7). Then, for each `posted_success` part in order
     (all of which already exist durably from step 8d, regardless of
     whether this step ever runs): claim =
     ...
     Otherwise send claim.payloadText via pushLineMessage() using
     claim.retryKey — the outbox is Push API only, never the reply API
     (§18.7), in this step or any other. On a successful send (or a 409
     replay with the same retry_key, §18.4), call
     mark_purchase_capture_notification_part_delivered(claim.id,
     ...
     while an earlier one remains undelivered). This same claim-then-push
     loop is what `after()` and the cron sweep also run — there is one
     outbox push worker, not a variant per timing (§18.6, §18.7), and it
     never needs step 8 to have run in the same request: any session
     already `'posted'` already has its complete part set, by construction.
```

### 12.1 Partial-success recovery (crash between step 3 and step 9)

**Required property, satisfied by construction**: a retry must resume posting the *same* frozen confirmation and must not create a second receipt, movement, or partial `posted_success` outbox state. Recovery re-enters at step 4 of §12 (recomputing `confirmationKey`) — it never re-runs step 2's advisory read or step 3's `begin_purchase_capture_confirmation` transition, because by the time a session reaches `confirming` that authoritative check has already passed once and the transition has already committed; recovery is only ever finishing steps 4-9, never re-deciding whether to start.
...
- **Step 8's idempotency**: retrying `complete_purchase_capture_posting` with the same `movementId` and byte-identical `postedSuccessPayloadTexts` either lands the transition-plus-part-set for the first time, or — if it already landed — is a no-op replay that returns the same rows and the same `retry_key`s (§12 step 8c-d). Retrying it with a *different* `movementId` than whatever the session already recorded refuses `invalid_state` rather than silently re-pointing an already-posted session at new content; retrying it with the same `movementId` but a genuinely different payload refuses `notification_identity_conflict` and leaves the session exactly as it was. Because (d) and (e) are one transaction, a crash cannot land one without the other — there is no intermediate state where the session reads `'posted'` but the `posted_success` parts don't fully exist, and no intermediate state where the parts exist but the session is still `'confirming'`.
...
- **What the user sees during retry**: nothing new is sent unless/until step 9 actually claims and pushes a part — a crash mid-sequence produces no visible LINE message (the last message the user saw was the `awaiting_confirmation` preview or a transient "กำลังยืนยัน..." ack); the eventual success message (or, after some bounded number of cron sweeps, an operator-visible stuck notice — **OWNER DECISION REQUIRED**, §24, on the exact bound and escalation path) arrives once recovery completes, delivered part-by-part via the `posted_success`/`stuck_escalation` notification's own claim-and-send parts, each with its own `retry_key` (§18).
- **When the session becomes terminal**: only inside step 8's own transaction (8e/8f), and only together with its complete `posted_success` part set (8d) — never earlier, and never one without the other. A crash before step 8's transaction commits leaves the session correctly non-terminal (`'confirming'`) and eligible for the recovery sweep, which simply calls `postPurchaseReceipt` again (idempotent replay, step 7) to rediscover `movementId` and then retries step 8 — there is no code path that reaches `'posted'` without also having a complete, durable outbox part set ready for cron/Push-API recovery.
- **Movement ID persistence/rediscovery**: recorded onto the session row inside step 8's transaction, alongside the part set, never separately. If a crash happens after step 7 (posting itself already succeeded, so `movementId` exists in `inventory_movements`) but before step 8 commits, the session still has no `movement_id` of its own — recovery *rediscovers* it, not regenerates it, by calling `postPurchaseReceipt` again (its dedupe lookup, §2.3, returns the existing `movementId` with `replayed:true`) and then calls step 8 with that rediscovered value, same as a fresh attempt.
...
| `purchase_capture_session_ingests.line_event_id` | Same LINE event id | Per purchase-capture ingest | New `UNIQUE(line_event_id)` (§8, PROPOSED, mirrors `0047:141`), **plus** an exact fingerprint comparison (session/ownership identity, `line_timestamp_ms`, `kind`, `raw_text`, `line_message_id`, `raw_message_id`) on every redelivery path in `open_purchase_capture_session`/`admit_purchase_capture_event` — a matching fingerprint is idempotent, any differing field refuses `line_event_conflict` (§16.4, §17, PROPOSED) |
...
| `session_generation` + `(source_type, source_id, sender_line_user_id)` | `gen_random_uuid()` at session open, plus the caller-supplied ownership triple | Per live session lifetime; identity/ownership re-check only — **never** reused as a push retry key | Session row uniqueness + `p_expected_generation` **and** `p_expected_source_type`/`p_expected_source_id`/`p_expected_sender_line_user_id` re-check under lock on every mutating (and the one content-returning read) RPC (§6.4, §16.4, §17, PROPOSED, mirrors `0047` and extends it with the explicit ownership triple) — a correct `session_generation` alone is insufficient; a mismatched ownership triple refuses `ownership_mismatch` even then |
...
| `movement_id` at posting-completion time | The `movementId` `postPurchaseReceipt` actually returned (fresh or replayed), passed by the caller and re-verified against the locked session's already-recorded `movement_id`, if any (§12 step 8c) | Per posting-completion attempt — `complete_purchase_capture_posting`'s own idempotency/ownership guard, and simultaneously the identity component of the `posted_success` part set it creates in the same transaction | `complete_purchase_capture_posting`'s in-transaction re-check under `FOR UPDATE`, plus the `posted_success` part set's own `UNIQUE(session_id, notification_kind, notification_version, part_index)` where `notification_version = movement_id` (§12 step 8, §16.4, §16.3, PROPOSED) — a mismatched `movement_id` refuses `invalid_state` before any part is touched |
...
| Application failure after receipt confirmation but before ledger posting (crash before step 7 completes) | Session stuck in `confirming` past a threshold | Cron sweep resumes posting using the same deterministic `confirmationKey`/derived `dedupeKey` (§12.1) |
| Application failure after P2C posting succeeds but before `complete_purchase_capture_posting`'s transaction commits (crash between step 7 and step 8) | Session still reads `'confirming'`, no `movement_id` recorded on the session row, no `posted_success` outbox rows exist for this movement (§12.1) | Cron sweep calls `postPurchaseReceipt` again (idempotent, §2.3, returns the existing `movementId` with `replayed:true`) and retries `complete_purchase_capture_posting` with that rediscovered `movementId` — the session transitions to `'posted'` and its complete `posted_success` part set is created together, in the first transaction that actually lands |
| Application failure after `complete_purchase_capture_posting` commits but before any part is pushed (crash between step 8 and step 9) | Session reads `'posted'`, `movement_id` is set, and the complete `posted_success` part set already exists — all durable, all from the same committed transaction (§12.1, §12 step 8) | Cron (or the next request) simply runs step 9's claim-then-push loop against the already-existing parts — no re-entry into step 8 is needed or attempted, because a `'posted'` session's part set is already guaranteed complete by construction |
| `complete_purchase_capture_posting` retried with a `movement_id` that does not match the one already recorded on an already-`'posted'` session | Session status check under lock (§12 step 8c) | Refused `invalid_state` — a `posted_success` part set is never created for, or attached to, a movement other than the one this session actually posted |
| `complete_purchase_capture_posting` retried with the same `movement_id` but a genuinely different `postedSuccessPayloadTexts` | The inline multipart-creation logic's identity-conflict check (§12 step 8d, §18.3) | Refused `notification_identity_conflict`; the whole transaction rolls back — the session stays exactly as it was (already `'posted'` with its original parts untouched, or, if this was the very first completion attempt, still `'confirming'` with no partial state left behind) |
| Push API failure on one claimed part | `pushLineMessage`'s existing 409/retry-key handling (§2.5, reused unmodified); `record_purchase_capture_notification_part_attempt` releases the part's lease back to `'failed'` (§18.4) | Cron (or the next claim call) claims that same part again via `claim_next_purchase_capture_notification_part` — same `retry_key`, same stored `payload_text`, every time, because neither changes across claims of the same row; earlier, already-delivered parts of the same logical notification are never re-sent, and a different logical notification (different `notification_kind`/`notification_version`) always has its own rows and keys, so it can never be treated as a retry of this one |
| An optional reply-API acknowledgement fails, or its outcome is ambiguous (unknown whether LINE received it) | Nothing — the acknowledgement has no row, no `retry_key`, and no durable state to check (§18.7) | No recovery action exists or is needed: no outbox part's `delivery_status` is affected either way, and the authoritative preview/result is still delivered afterward through the Push API outbox exactly as if the acknowledgement had never been attempted |
| A reply token is already consumed or expired by the time an acknowledgement would be sent | The reply-API call itself fails or is skipped (§18.7) | The acknowledgement is simply not sent; processing continues unaffected — no outbox part waits on, or is gated by, a reply token's availability |
...
- `20260805020000_purchase_capture_confirm_post.sql` (Slice C — `begin_purchase_capture_confirmation`, `complete_purchase_capture_posting`; no new tables, reuses Slice B's outbox internals for `posted_success`/`stuck_escalation`, §21)
...
- A **physical part** is exactly one row, and exactly one `pushLineMessage()` call — **the durable outbox is Push API only, never the reply API** (§18.7). One row = one LINE message = one immutable `retry_key`, never more. A logical notification that needs three LINE messages to render (a long preview, chunked the same way this repo already chunks other long reports) is three rows, three `retry_key`s, sent in order, all three via `pushLineMessage()`.
...
- `open_purchase_capture_session(p_source_type, p_source_id, p_sender_line_user_id, p_opened_line_event_id, p_line_timestamp_ms, p_raw_text, p_line_message_id, p_raw_message_id)` — mirrors `open_physical_inventory_session`; returns `already_open` if the partial unique index would be violated; a redelivered `p_opened_line_event_id` is idempotent only when its fingerprint (source/sender identity plus timestamp/raw_text/message ids) matches the original admission exactly — a differing fingerprint under the same event id refuses `line_event_conflict` rather than silently accepting or overwriting different content (§8.2, §13).
- `admit_purchase_capture_event(p_session_id, p_expected_generation, p_expected_source_type, p_expected_source_id, p_expected_sender_line_user_id, p_line_event_id, p_line_timestamp_ms, p_kind, p_raw_text, p_line_message_id, p_raw_message_id)` — mirrors `admit_physical_inventory_event`, extended with the explicit ownership triple (§6.4, §17): re-verifies `(source_type, source_id, sender_line_user_id)` against the locked session before any mutation, refusing `ownership_mismatch` otherwise. Handles `item`/`costs` admits and the multi-message close-boundary-setting admit (`p_kind = 'close'`). A redelivered `p_line_event_id` is idempotent only when its fingerprint (session identity plus timestamp/kind/raw_text/message ids) matches the original admission exactly — a differing fingerprint refuses `line_event_conflict` rather than silently accepting or overwriting different content (§13).
- `close_purchase_capture_open_event(p_session_id, p_expected_generation, p_expected_source_type, p_expected_source_id, p_expected_sender_line_user_id, p_opened_line_event_id)` — **new (§8.4)**, extended with the ownership triple (§6.4, §17): re-verifies `(source_type, source_id, sender_line_user_id)` against the locked session before any mutation, refusing `ownership_mismatch` otherwise. Sets the close boundary from the *already-persisted* opening ingest row for the one-message (Header+Items+Costs+Close-in-a-single-message) case, without inserting a second ingest row for the same `line_event_id`; idempotent on redelivery (the boundary fields are immutable once set, so a repeat call observes the existing boundary rather than erroring).
- `get_purchase_capture_finalize_candidate(p_session_id, p_expected_generation, p_expected_source_type, p_expected_source_id, p_expected_sender_line_user_id)` — mirrors `get_physical_inventory_finalize_candidate`, extended with the ownership triple (§6.4, §17): although this RPC is a read with no side effects, it returns the session's full ingest content, so the ownership check is part of the lookup's own `WHERE` clause rather than a separate post-check — a caller with a correct `session_id`/`session_generation` but the wrong `source_type`/`source_id`/`sender_line_user_id` sees no row and is refused `ownership_mismatch`, distinct from `generation_conflict`/not-found. Returns the current ingest set, in canonical `(line_timestamp_ms, line_event_id)` order (never `ingest_ordinal`/admission order — that ordering is what the authoritative `ingest_set_hash` below is also computed in) + revision + hash + quiet/deadline eligibility flags, for the caller to run the parser adapter (§9) against; **Slice B's parser adapter must still sort the returned set defensively rather than trust this ordering as a contract.** **Slice A's own tests exercise this RPC directly and assert eligibility is computed correctly; Slice A does not call any RPC that writes `awaiting_confirmation`.**
- `cancel_purchase_capture_session(p_session_id, p_expected_generation, p_expected_source_type, p_expected_source_id, p_expected_sender_line_user_id)` — extended with the ownership triple (§6.4, §17): re-verifies `(source_type, source_id, sender_line_user_id)` against the locked session before any mutation, refusing `ownership_mismatch` otherwise. Any of `open/closing` → `cancelled` in Slice A (the `awaiting_confirmation` branch of this same RPC becomes reachable once Slice B exists, but the RPC signature and CHECK-guarded transition table are defined once, here).
...
- `finalize_purchase_capture_session(p_session_id, p_expected_generation, p_expected_source_type, p_expected_source_id, p_expected_sender_line_user_id, p_expected_ingest_revision, p_expected_ingest_hash, p_assembly_status, p_receipt_id_or_null, p_draft_revision_or_null, p_preview_payload_texts_or_null, p_fail_reason_or_null)` — mirrors `finalize_physical_inventory_session`; under the session `FOR UPDATE` lock, refuses `ownership_mismatch` if `(source_type, source_id, sender_line_user_id)` or `session_generation` do not match the locked row before any transition or outbox mutation. Input contracts fail closed: on the `→ failed_closed` branch, `receipt_id`, `draft_revision`, and preview payloads must be `NULL` and `fail_reason` must be present (or the documented default); any success-only argument is rejected. On the `→ awaiting_confirmation` branch, `receipt_id`, `draft_revision`, and a non-empty preview payload array are required, `fail_reason` must be `NULL`, and the referenced receipt is locked and verified (`status = draft`, matching `draft_revision`, `document_namespace = line-text`, `document_key = opened_line_event_id`, ownership fields match the session) before session transition, lifecycle insert, or notification creation. Transitions `closing` → `awaiting_confirmation` **only when the caller supplies a real, non-null `receipt_id`/`draft_revision` already written by a preceding `upsert_purchase_receipt_draft` call** (§2.2) — the RPC does not accept a stub/placeholder outcome; a `NULL` receipt_id is only valid on the `→ failed_closed` branch. On the `awaiting_confirmation` branch, in the same transaction, also creates the session's first `preview_ready` outbox part set from `p_preview_payload_texts_or_null` (an ordered `text[]`, one element per LINE message the rendered preview needs — usually one, occasionally more for a long preview; via the same internal logic `create_purchase_capture_notification_parts` uses, §16.3) — there is nothing to supersede yet, since this is the document's first preview. This is the RPC that Slice A's schema defines space for but Slice A never calls with an `awaiting_confirmation` target (§21).
...
- `complete_purchase_capture_posting(p_session_id, p_expected_generation, p_movement_id, p_posted_success_payload_texts)` — **new, replaces the earlier `mark_purchase_capture_session_posted`, which is removed rather than kept alongside this one** (§12 step 8). The sole authoritative posting-completion gate: in one transaction, locks the session `FOR UPDATE`, re-checks ownership/generation, re-checks status (`'confirming'` → proceed; already `'posted'` with the same `movement_id` → idempotent replay; already `'posted'` with a different `movement_id`, or any other status → refuse `invalid_state`), creates the complete `posted_success` part set inline using the same atomic multipart-creation logic `create_purchase_capture_notification_parts` uses (`notification_kind = 'posted_success'`, `notification_version = p_movement_id`, one immutable `retry_key` per part, same identity-conflict fail-closed rule — a payload conflict refuses `notification_identity_conflict` and rolls back the whole transaction, including the status transition), and only then transitions the session to `'posted'` with `movement_id = p_movement_id` recorded — all committed together, or none of it. This is what guarantees a session can never read `'posted'` without its complete, durable `posted_success` part set already existing, and that a `posted_success` part can never exist for a `movement_id` not attached to that session (§12.1). Delivering the parts (claiming and pushing each one) remains a separate step performed afterward by the shared push worker (§12 step 9, §18.4, §18.7) — this RPC's only job is the atomic transition-plus-part-set commit, never the network call.
...
- **Source/sender ownership checks inside the authoritative mutation boundary**: every admit/close/cancel/finalize/confirm/post RPC re-verifies `(source_type, source_id, sender_line_user_id, session_generation)` under row lock, never trusting the caller's claim without a DB-side check (§6.4) — this is the same discipline `0047`/`0052` already apply. **In Slice A** (§16.4), `admit_purchase_capture_event`, `close_purchase_capture_open_event`, and `cancel_purchase_capture_session` all refuse `ownership_mismatch` before any mutation if the caller's `p_expected_source_type`/`p_expected_source_id`/`p_expected_sender_line_user_id` disagree with the locked session row, even when `p_session_id`/`p_expected_generation` are both correct — a correct session id and generation are necessary but never sufficient. `get_purchase_capture_finalize_candidate`, though a read with no side effects, is included for the same reason, since it returns the session's full ingest content: the ownership triple is part of its lookup's own identity, not a separate trusted parameter.
- **A repeated LINE event id is idempotent only under an exact fingerprint match.** `open_purchase_capture_session` and `admit_purchase_capture_event` (§16.4) both compare every immutable field of a redelivered `line_event_id` — session/ownership identity, `line_timestamp_ms`, `kind`, `raw_text`, `line_message_id`, `raw_message_id` — against the originally-admitted row before treating the redelivery as idempotent. A same-id, differing-content redelivery is refused `line_event_conflict` and never silently accepted, overwritten, or merged — this closes what would otherwise be a content-substitution gap under LINE's own guarantee that event ids are immutable but a defensively-guarded one, exactly as `0047`'s `line_event_conflict` handling already is (§2.5, §8.2).
...
- A **physical part** is one row in `purchase_capture_notifications` (§16.3), one exact `payload_text`, and one immutable `retry_key`. Delivering a logical notification means delivering every one of its parts, each via its own `pushLineMessage()` call — **the outbox transport is Push API only; the reply API is never used to deliver a part**, per §18.7's transport lock. `part_index` (`0`-based) fixes the order; `part_count` is the same on every row of one logical notification's part set.
- **One retry key per physical push request, always.** No row's content is ever sent across more than one HTTP request, and no HTTP request to LINE is ever made without a row (and therefore a key) behind it. This is the rule that closes §18.1's bug — and it only holds because delivery is Push-API-only: LINE's reply token has no equivalent per-request retry key, cannot be reused across multiple messages the way this design would need, and is excluded from the durable delivery contract entirely (§18.7).

### 18.3 Multipart creation is atomic

`create_purchase_capture_notification_parts(session_id, notification_kind, notification_version, payload_texts)` (§16.3, §16.4) — or the identical inline logic `complete_purchase_capture_posting` runs in its own transaction for `posted_success` (§12 step 8d, §16.4) — is invoked once, by whichever code path first needs the notification to exist (`finalize_purchase_capture_session`, `replace_purchase_capture_draft`, or `complete_purchase_capture_posting`; `stuck_escalation`'s eventual caller, §21) — never incrementally, never one part at a time. Given the complete ordered array of message texts to send:
...
- **Webhook reply vs. push — corrected, §18.7**: an earlier draft of this section said a claimed part could be sent via the reply API while still inside the original webhook request, "since a reply send does not depend on `X-Line-Retry-Key` collision semantics." That was wrong and is removed: LINE's reply token can be used at most once per webhook event and carries no retry key at all, so a multipart notification cannot be delivered one part per reply call — the token is spent on part 1, and every later part has no reply call left to use and no retry key to fall back on if a reply-based send's outcome is ambiguous. **Every claimed part, in every code path — inside the webhook request, in `after()`, or from cron — is delivered exclusively through `pushLineMessage()`**, claimed first (§18.4) and marked delivered afterward through the same two RPCs regardless of which code path is running; there is one claim-and-settle contract and one transport, not one per timing. The reply API's only remaining role is the optional, non-durable acknowledgement described in §18.7 — it never substitutes for, defers, or skips a part's push delivery.
...
### 18.7 Transport contract: Push API only for the durable outbox, Reply API for ephemeral acknowledgements only

**PROPOSED, and a direct correction of an earlier draft's "reply or push" language for a claimed part, which was self-contradictory.** LINE's reply token can be used at most once per webhook event and does not carry an `X-Line-Retry-Key`-equivalent at all — the repo's own outbound convention (§2.5) only ever documents `X-Line-Retry-Key` on `pushLineMessage`. `replyLineMessages()`, as it exists in the repo today, batches up to five message objects into that one single reply call; `pushLineMessage()` sends exactly one text message per HTTP request, with its own retry key, every time. Delivering a multipart outbox notification one part per reply call therefore cannot work: part 1 consumes the event's one reply token, and every later part has no reply call left to make and no key to fall back on if an earlier attempt's network outcome was ambiguous — reply delivery has no durable, exactly-once recovery path the way a push `retry_key` provides one.

**The V1 transport contract is locked to one unambiguous rule: every `purchase_capture_notifications` part is delivered through `pushLineMessage()`, and only `pushLineMessage()` — never the reply API.**

- One outbox part row maps to exactly one Push API HTTP request, using that row's own immutable `retry_key`, in every code path that can deliver one — inside the original webhook request, in the deferred `after()` continuation, and from the cron sweep (§18.4, §18.6). There is exactly one delivery worker and one claim-and-settle contract; this design does not define a separate synchronous-reply delivery path and a separate asynchronous-push delivery path for the same logical notification.
- The claim/lease and ordered-delivery contracts of §18.4 apply only to this Push API path — there is no reply-API equivalent of a claim, because the reply API is never used to settle an outbox row.
- A part is marked `'delivered'` only on a Push API 2xx response, or a Push API 409 that reuses that exact part's own `retry_key` (§18.4) — signifying LINE already accepted an earlier attempt of this exact part. **A part is never marked delivered on the basis of a reply-API call, successful or ambiguous or failed** — the reply API has no row, no retry key, and no place anywhere in the outbox's `delivery_status` state machine.
- Cron recovery always retries an undelivered part through the Push API, using that row's own stored `retry_key` and `payload_text` (§18.4, §18.6) — never through a reply call, which would require a live webhook event's reply token that a background sweep does not have.

**The Reply API remains available, but strictly outside this durable contract, for one narrow purpose: an optional, ephemeral, non-durable acknowledgement**, sent at most once per webhook event that warrants one — short text such as "รับข้อมูลแล้ว กำลังประมวลผล" or "กำลังตรวจสอบใบซื้อ", acknowledging that the message was received before its authoritative content is ready or before any outbox part has been claimed. An acknowledgement:

- is never a `preview_ready`, `posted_success`, or `stuck_escalation` notification, and never creates a `purchase_capture_notifications` row of any kind — it has no identity, no `notification_version`, no `retry_key`, and is not tracked by the outbox at all;
- never marks any outbox part `'delivered'` and never touches `delivery_status` on any row — sending it, or failing to send it, has zero effect on database correctness;
- consumes the webhook event's one reply token if it is sent at all; the token is never reused for anything else, and no later step depends on the acknowledgement having succeeded — an ambiguous or outright failed reply attempt is simply absorbed (no acknowledgement text reaches the user) and processing continues exactly as it would have if no acknowledgement had been attempted;
- is always followed by the authoritative preview/result being delivered through the Push API outbox (§18.4), **even when the full computation actually finished within the original webhook request and an immediate reply would have been technically possible.** This is deliberate, not an oversight: it keeps the durable delivery model identical regardless of whether processing completes synchronously in the webhook request, in `after()`, or during cron recovery — inventing a reply-based fast path for the "finished fast enough" case would mean the outbox's ordering, claim/lease, supersession, and exactly-once guarantees (§18.2-§18.5) simply would not apply to that case, which this design does not accept.

**A valid, unconsumed reply token must never cause the durable outbox delivery path to be skipped, deferred, or bypassed, and must never be treated as equivalent to marking a part delivered.** The acknowledgement, if sent, is fire-and-forget UX; the Push-API-delivered outbox parts are the only durably-tracked, authoritative delivery of a notification's actual content, in every code path, unconditionally.
...
- Duplicate open event, identical fingerprint (same `opened_line_event_id`, same source/sender/timestamp/raw_text redelivered) → idempotent, no second session.
- Duplicate item event, identical fingerprint → idempotent, no duplicate item admitted.
- **Conflicting event reuse — every differing field, both RPCs**: same `opened_line_event_id`/`line_event_id` but a different `raw_text`, `line_timestamp_ms`, `kind` (admit only), `source_type`/`source_id`/`sender_line_user_id` (open only), `line_message_id`, or `raw_message_id` → refused `line_event_conflict` in every case, never silently accepted, overwritten, or merged; assert the originally-admitted row's content is byte-identical after every rejected redelivery (§13, §16.4).
- Two concurrent item admissions → both admitted, correct `ingest_revision` sequencing, no lost update.
- **Concurrent identical duplicate (open and admit)** → exactly one row inserted, the other call returns idempotent replay (`duplicate_open_event`/`duplicate_event`), never two rows and never an error on the losing side.
- **Concurrent conflicting duplicate (open and admit)** → exactly one call succeeds (inserts), the other refuses `line_event_conflict`; exactly one row exists for that event id, never both contents, never neither.
...
- **Ownership mismatch — every mutating RPC plus the read**: `admit_purchase_capture_event`, `close_purchase_capture_open_event`, `cancel_purchase_capture_session`, and `get_purchase_capture_finalize_candidate`, each called with a **correct** `session_id`/`session_generation` but a **wrong** `source_type`, `source_id`, or `sender_line_user_id` individually → refused `ownership_mismatch` in every case (distinct from `generation_conflict`); assert no ingest, boundary, cancellation, or lifecycle mutation occurred from any rejected call, and that the read returns no content (§6.4, §16.4).
- Sender ownership isolation — sender B cannot admit/confirm sender A's session, enforced by the `ownership_mismatch` checks above, not merely by the webhook handler routing correctly.
- Source isolation — the same sender in two different LINE groups gets two independent sessions.
- Stale generation — an RPC call carrying a superseded `session_generation` is refused `generation_conflict`, checked before the ownership triple.
- Stale revision/hash — finalize refused if the ingest set changed since the candidate was read (§8.3).
- **Ingest-set hash is admission-order independent**: two admission orderings of the same session's ingest rows (natural timestamp order vs. reverse-of-timestamp admission order) both produce a hash equal to a reference computed directly in canonical `(line_timestamp_ms, line_event_id)` order — and, for the reverse-admission-order case, a hash **different** from a reference computed in `ingest_ordinal` (admission) order, positively proving `ingest_ordinal` is not what the hash is keyed on. `get_purchase_capture_finalize_candidate`'s own `ingest_set_hash` matches `purchase_capture_compute_ingest_set_hash` in both cases (§8.2, §16.4).
- **Hash sensitivity**: changing any one of `line_timestamp_ms`, `line_event_id`, `kind`, or `raw_text` on one ingest tuple changes the session's `ingest_set_hash` (§8.2).
...
- **A three-part preview completed entirely within the original webhook request still creates three outbox rows and performs three ordered Push API calls** — simulate a `closing`→`awaiting_confirmation` finalize whose preview needs 3 messages, all handled synchronously in one webhook request/response cycle → assert exactly 3 `purchase_capture_notifications` rows exist and exactly 3 `pushLineMessage()` calls occurred, never a single batched reply-API call and never fewer than 3 push calls (§18.7).
- **Every part uses a different `retry_key`** — for the same 3-part preview, assert all 3 `retry_key` values are distinct (§18.2, §18.7).
- **A reply token is used at most once, for an optional acknowledgement only** — simulate the same webhook request, asserting the reply API is called at most one time (the ephemeral acknowledgement, if sent) and never once per outbox part (§18.7).
- **No outbox part is marked delivered by the acknowledgement reply** — after the ephemeral acknowledgement is sent (successfully or not), assert every outbox part's `delivery_status` is still `'pending'` until its own, separate Push API call settles it (§18.7).
- **An ambiguous or failed reply-API acknowledgement does not alter outbox delivery state** — simulate the acknowledgement send throwing, timing out, or returning an error → assert no `purchase_capture_notifications` row was created, updated, or otherwise touched by that failure, and that claim-and-push delivery of the outbox parts proceeds identically to the case where the acknowledgement succeeded (§18.7).
- **Push API 200 marks only that part delivered** — a successful push of part 1 marks part 1 `'delivered'`; parts 0 and 2 are unaffected by that specific call (§18.4, §18.7).
- **Push API 409 for that part's own `retry_key` marks only that part delivered** — a 409 response reusing part 1's `retry_key` marks part 1 `'delivered'`; it has no effect on any other part's row, and it is never interpreted as settling a reply-API acknowledgement (there is none to settle) (§18.4, §18.7).
- **Webhook, `after()`, and cron paths use the same outbox push worker** — an architecture-style test (mirroring `architecture.test.ts`, §2.1) asserting the webhook-synchronous code path, the `after()` continuation, and the cron sweep all call the same shared claim-then-push function (`notification-push-worker.ts`, §21) rather than each containing its own copy of the claim/push/settle loop or its own reply-based shortcut.
- **Multipart notification delivery remains recoverable after the webhook's reply token has expired or already been used** — deliver part 0 of a 3-part notification during the original webhook request (consuming or expiring that request's one reply token in the process, if an acknowledgement was sent), then simulate the webhook request ending before parts 1-2 are delivered → a later cron pass, with no reply token available at all, successfully claims and pushes parts 1 and 2 to completion using only their own `retry_key`s — proving delivery never depended on the reply token surviving past the original request (§18.4, §18.7).
- **Crash after P2C posting but before `complete_purchase_capture_posting`'s transaction commits** — call `postPurchaseReceipt` to completion (a real `movementId` exists in `inventory_movements`), then simulate the process dying before calling `complete_purchase_capture_posting` at all → assert the session still reads `'confirming'`, has no `movement_id` recorded, and no `posted_success` rows exist; then run the recovery sweep → assert `postPurchaseReceipt` replays the same `movementId` (`replayed:true`, no second movement) and `complete_purchase_capture_posting` completes safely, landing the session at `'posted'` with its complete part set, all in the retry (§12.1, §12 step 8).
- **Crash after `complete_purchase_capture_posting`'s transaction commits but before any Push API call** — call `complete_purchase_capture_posting` to a real commit, then simulate the process dying before step 9's claim-then-push loop ever runs → assert the session already reads `'posted'` with `movement_id` set, and the complete `posted_success` part set already exists, every part `'pending'`; then run cron recovery → assert it claims and pushes every part to `'delivered'` using the rows that were already there, with no re-entry into step 8 (§12.1, §12 step 9).
- **Repeated `complete_purchase_capture_posting` with the same movement and byte-identical payload is idempotent** — call it twice with identical `movement_id`/`postedSuccessPayloadTexts` → the second call returns the same session state (`'posted'`, same `movement_id`) and the same outbox rows/`retry_key`s as the first; no second transition, no second part set, no error (§12 step 8c-d, §12.1).
- **A conflicting `movement_id` on an already-posted session fails closed** — call `complete_purchase_capture_posting` once with `movement_id = A` (succeeds), then again with `movement_id = B` for the same session → refused `invalid_state`; the session's `movement_id` remains `A`, and no `posted_success` part set is ever created for `B` against this session (§12 step 8c, §14).
- **A conflicting payload for the same movement fails `notification_identity_conflict`** — call `complete_purchase_capture_posting` once with `movement_id = A` and one payload (succeeds), then again with the same `movement_id = A` but a different `postedSuccessPayloadTexts` → refused `notification_identity_conflict`; the session's original state and original part set/`retry_key`s are unchanged (§12 step 8d, §14).
- **No posted session exists without a complete `posted_success` part set** — an invariant test: for every session in the test suite's fixture set that reaches `status = 'posted'` (via any path — normal flow, crash-recovery, or a stress test interleaving many concurrent completion attempts), assert a `purchase_capture_notifications` row exists for `(session_id, 'posted_success', session.movement_id)` at every `part_index` from `0` to `part_count - 1` with no gaps — there is no code path, including a partial failure, that can produce a `'posted'` session with a missing, partial, or absent part set (§12 step 8, §12.1).
...
- **Files/components**: new `supabase/migrations/<timestamp>_purchase_capture_confirm_post.sql` adding only `begin_purchase_capture_confirmation`/`complete_purchase_capture_posting` RPCs (§16.4) — **no new tables and exactly one new notification-producing RPC**, `complete_purchase_capture_posting` (which runs Slice B's multipart-creation logic inline, in its own transaction, rather than calling `create_purchase_capture_notification_parts` as a separate round-trip — this is the fix for the posting/outbox atomicity gap this slice closes); `src/lib/purchase-capture/confirm-flow.ts` implementing §12's exact sequence, including step 2's **advisory-only** blocker read (used purely to word an early refusal reply; the actual gate is `begin_purchase_capture_confirmation`, called in step 3, which is the only thing permitted to transition the session), an optional ephemeral reply-API acknowledgement per §18.7 (sent, if at all, only after `complete_purchase_capture_posting`'s transaction has committed, §12 step 9), and the `ตรวจใบซื้อใหม่`/`ยืนยันซื้อ`/`ยกเลิกซื้อ` LINE command dispatch; posting success calls `complete_purchase_capture_posting` once (§12 step 8), then invokes Slice B's shared `notification-push-worker.ts` to claim and deliver the parts that call already committed — no new push/reply logic is written in this slice, only one new atomic-completion RPC and reuse of Slice B's push worker; extending the Slice A cron route to also perform §12.1's recovery sweep (retrying `postPurchaseReceipt` then `complete_purchase_capture_posting` for any session stuck in `confirming`) and to widen §18's already-existing outbox-redelivery sweep (also using the same shared push worker) to also cover `posted_success`/`stuck_escalation` rows; the single new call site into `webhook-service.ts`'s `processOne` chain (§2.7) — this is the only slice that touches the existing webhook file, and only by adding one call, not modifying existing branches.
- **Migration dependency**: Slices A and B.
- **Tests**: §19.2's confirm-vs-replace race tests (replace-wins-first and confirmation-wins-first outcomes, no deadlock, receipt/session revision agreement); §19.3 in full (posting-specific, confirm-gate, `complete_purchase_capture_posting` atomicity, and `posted_success`-scoped outbox cases); the crash-recovery cases exercised explicitly (kill the process between P2C posting and `complete_purchase_capture_posting`'s commit, and again between that commit and the first push attempt, in a test harness, then run the recovery sweep for each).
- **Definition of Done**: the full journey in the task's stated goal works end-to-end against a disposable local Postgres, including: a blocked draft correctly refusing `ยืนยันซื้อ` via `begin_purchase_capture_confirmation`'s own authoritative check (not the app-side read); `ตรวจใบซื้อใหม่` clearing the block atomically; the confirm-vs-replace race resolving deterministically in both directions with no deadlock, no stale draft ever reaching `confirming`, and no receipt ever modified after its session entered `confirming`; **no session ever reads `'posted'` without its complete `posted_success` part set already existing, and no `posted_success` part ever exists for a `movement_id` not attached to its session** (`complete_purchase_capture_posting`'s atomicity, §12 step 8, §12.1); and both crash-recovery cases (before and after that RPC's commit). `webhook-service.ts`'s existing routing order/behavior for every other feature is unchanged (regression-tested by the existing suite, §2.4's test list).
...
5. Once confirmed, `ยืนยันซื้อ` posts exactly one `PURCHASE_RECEIPT` movement with exactly one line per item, increasing the MAIN balance by exactly the declared quantities, exactly once, even under redelivery/retry/crash (§12, §14 all pass as tests, §19 in full). **A session never reads `'posted'` without its complete `posted_success` outbox part set already existing, and a `posted_success` part never exists for a `movement_id` not attached to that session** — `complete_purchase_capture_posting` (§12 step 8, §16.4) transitions the session and creates the full part set in one transaction, so a crash between P2C posting and this commit leaves the session correctly non-terminal and recoverable, and a crash after this commit but before any part is pushed leaves a `'posted'` session whose complete part set is already durable and cron-recoverable (§12.1, §19.3's crash-recovery tests pass in full).
...
The durable-session, parser, persistence, and ledger layers this flow needs are either already fully built and Production-verified (P2B parser, P2B persistence, P2C ledger — §2.1-§2.3, §3) or are a direct, low-risk adaptation of a pattern already running successfully in Production for a structurally identical problem (the Physical Inventory close barrier — §2.4, §8, extended with the one-message inline-close path, §8.4). No part of this design requires inventing a new safety mechanism from scratch; every idempotency and ownership guarantee in §13/§17 is either reused verbatim or built by copying an existing, tested shape. Seven correctness gaps found across earlier drafts of this design are now closed: the confirm-gate is authoritative and race-safe (`begin_purchase_capture_confirmation`, §12, §16.4); the recheck path is a single atomic transaction rather than two separately-committed steps, so a receipt can never be replaced without its session revision advancing in the same commit, and can never be modified after its session has entered `confirming` (`replace_purchase_capture_draft`, §12.2, §16.4); the notification outbox stores the exact, immutable content to send, not merely a hash, and fails closed on a same-identity/different-content conflict rather than silently overwriting it (§16.3, §18); an obsolete undelivered preview is marked `'superseded'` in the database the instant a newer one exists, so it can never be sent by a late-arriving retry (§18); notification delivery is scoped to one `retry_key` per physical LINE push request rather than one key per logical (possibly multi-message) notification, with ordered claim-and-send recovery, so a chunked preview can no longer have its later messages 409-suppressed by a key already spent on its first message (§16.3, §18.2-§18.4); the durable outbox is locked to the Push API exclusively — the reply API's single-use, no-retry-key token can no longer be asked to deliver more than one part of a multi-message notification, and it is used, if at all, only for an optional, non-durable acknowledgement that never substitutes for or gates a part's actual delivery (§18.7); and posting completion is now a single atomic transaction, `complete_purchase_capture_posting`, rather than a session-status write followed by a separately-committed notification-creation call — a session can no longer reach `'posted'` with no durable `posted_success` parts behind it for cron to recover, because the two now commit together or not at all (§12 step 8, §12.1, §16.4).
