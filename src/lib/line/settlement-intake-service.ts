...
export const SETTLEMENT_CMD_RE =
  /^(?:ส่งเงิน|ปิดยอด)\s+(\d{1,2}\/\d{1,2}\/(?:25)?\d{2})(?:\s+(\d+(?:\.\d+)?)\s*(?:บาท)?)?\s*$/;
...
const BARE_AMOUNT_RE = /^(\d+(?:\.\d+)?)\s*(?:บาท)?$/;
...
export function parseSettlementCommand(text: string): string | null {
  ...
  const m = firstLine.match(SETTLEMENT_CMD_RE);
  return m ? m[1] : null;
}
...
export function parseSettlementAmounts(text: string): ParsedAmounts {
  ...
  if (named.transfer !== null || named.cash !== null || named.expenses !== null || named.labor !== null) {
    return named;
  }

  const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
  for (const line of lines) {
    const command = line.match(SETTLEMENT_CMD_RE);
    if (command?.[2]) {
      return { transfer: parseFloat(command[2]), cash: null, expenses: null, labor: null };
    }
  }
  for (const line of lines) {
    if (SETTLEMENT_CMD_RE.test(line)) continue;
    const bare = line.match(BARE_AMOUNT_RE);
    if (bare) {
      return { transfer: parseFloat(bare[1]), cash: null, expenses: null, labor: null };
    }
  }
...
export function hasAnyAmount(text: string): boolean {
  ...
  return (
    amounts.transfer !== null ||
    amounts.cash !== null     ||
    amounts.expenses !== null ||
    amounts.labor !== null
  );
}
