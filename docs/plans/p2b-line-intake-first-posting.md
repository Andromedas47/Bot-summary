...
9. Notification: notif = get_or_create_purchase_capture_notification({
     sessionId, kind: 'posted_success', version: postResult.movementId }) —
     idempotent get-or-insert keyed on (session_id, kind, version), §16.3/§18
     — returning the row's immutable retry_key (freshly generated the first
     time, reused verbatim on every retry). Send via LINE's reply API if
     still in the original request/response cycle, otherwise push using that
     retry_key. On a successful send (or a 409 replay with the same key,
     §18), call mark_purchase_capture_notification_delivered(notif.id);
     record_purchase_capture_notification_attempt(notif.id, error) on any
     failed attempt so the row stays eligible for cron recovery (§18).
```
...
- **What the user sees during retry**: nothing new is sent unless/until the sequence reaches step 9 — a crash mid-sequence produces no visible LINE message (the last message the user saw was the `awaiting_confirmation` preview or a transient "กำลังยืนยัน..." ack); the eventual success message (or, after some bounded number of cron sweeps, an operator-visible stuck notice — **OWNER DECISION REQUIRED**, §24, on the exact bound and escalation path) arrives once recovery completes, using the `posted_success`/`stuck_escalation` notification row's retry_key (§18).
...
2. Run the unchanged pure parser (§9.1 steps 1-4) and identity resolution
   ...
   taken directly.
3. Render the preview text/payload snapshot from that same computed result
   (p_preview_payload_snapshot, §16.3, §18) — in memory, no DB write yet.
...
13. Create the new preview_ready outbox row (session_id, 'preview_ready',
    new draft_revision, p_preview_payload_snapshot) via the same get-or-
    create logic used elsewhere (§16.3, §18) — which, for this kind, also
    supersedes any older undelivered preview_ready row for this session in
    the same statement (§18).
...
| `(session_id, notification_kind, notification_version)` | E.g. `(session, 'preview_ready', draft_revision)`, `(session, 'posted_success', movement_id)`, `(session, 'stuck_escalation', escalation_version)` (§18) | Per logical notification — a distinct row, a distinct immutable `retry_key`, and a distinct immutable `payload_snapshot`, for every distinct piece of content that needs delivering | New `purchase_capture_notifications` `UNIQUE(session_id, notification_kind, notification_version)`, with a server-computed `payload_hash` guarding against a same-identity/different-content call (`notification_identity_conflict`, §16.3, §16.4, §18, PROPOSED) |
...
| Repeated `ตรวจใบซื้อใหม่` (sent twice, registry unchanged between calls) | Each call is its own `replace_purchase_capture_draft` transaction against the same `documentKey` (§2.2, §12.2) | Second call re-saves identical content as a new `draft_revision` (0052's full-replace is content-driven, not skipped just because nothing changed); a new `preview_ready` outbox row is created for that new revision and supersedes the previous one, so staff only ever see the latest preview as deliverable, never a stale duplicate |
...
| LINE reply/push failure | `pushLineMessage`'s existing 409/retry-key handling (§2.5, reused unmodified); the notification row's `delivery_status` stays `'pending'`/`'failed'` (§18) | Cron selects the durable eligible `purchase_capture_notifications` row (`delivery_status IN ('pending','failed')`), re-verifies it is still eligible immediately before sending, and retries with that row's own `retry_key` and its own stored `payload_snapshot` — the same key and the same exact content every time, because neither changes once the row exists; a different logical notification (different `notification_kind`/`notification_version`) always has its own row and its own key, so it can never be treated as a retry of this one |
...
| A worker already holding a reference to a preview row that gets superseded mid-flight | The worker re-reads the row's live `delivery_status` (or re-calls `get_or_create_purchase_capture_notification` with the same identity) immediately before its own send attempt (§18) | If the row now reads `'superseded'` or `'delivered'`, the worker aborts and does not send — its earlier-loaded copy was a snapshot, not a standing authorization |
| `get_or_create_purchase_capture_notification` called twice for the same identity with genuinely different content | Server-computed `payload_hash` comparison against the already-stored row (§16.4, §18) | The second call is refused `notification_identity_conflict`; the stored `payload_snapshot`/`payload_hash` are never overwritten. This should not happen under correct application code (each identity is only ever constructed from one deterministic render), so a conflict here indicates an application bug, not a normal operational case |
...
### 16.3 Table (Slice B) — durable notification outbox

**PROPOSED**, replacing the earlier per-session retry-key columns (§18). Introduced in **Slice B**, not Slice C — Slice B is the first slice that produces a `preview_ready` notification (from `finalize_purchase_capture_session` and `replace_purchase_capture_draft`), so the outbox and its general-purpose RPCs must exist there; Slice C only adds new *callers* (`posted_success`, `stuck_escalation`), not new schema (§21).

- **`purchase_capture_notifications`** — `id` (uuid pk), `session_id` (FK → `purchase_capture_sessions`), `notification_kind` (CHECK IN `'preview_ready','posted_success','stuck_escalation'`), `notification_version` (text — the exact `draft_revision` for `preview_ready`, `movement_id` for `posted_success`, one fixed escalation identifier for `stuck_escalation`), `retry_key` (uuid, `DEFAULT gen_random_uuid()`, immutable once set), `payload_snapshot` (`jsonb NOT NULL` — an **ordered array of the exact Unicode message strings** to send, one element per LINE message; `["..."]` for a single-message notification, `["...","..."]` if the content had to be chunked across multiple LINE messages, mirroring the existing chunk-limit handling this repo already applies elsewhere for long reports), `payload_hash` (`text NOT NULL`, server-computed as `encode(digest(payload_snapshot::text, 'sha256'), 'hex')` — never caller-supplied, so a row's hash always genuinely matches its own snapshot), `delivery_status` (CHECK IN `'pending','delivered','failed','superseded'`, `DEFAULT 'pending'`), `attempt_count` (`DEFAULT 0`), `last_attempt_at`, `delivered_at`, `superseded_at`, `last_error`, `created_at`, `updated_at`.
- **Unique identity**: `UNIQUE(session_id, notification_kind, notification_version)` — this is the row-level idempotency key described in §13. A row is created **once** per distinct logical notification and never for a repeat of the same content; a new `draft_revision` after `ตรวจใบซื้อใหม่` is a *different* `notification_version`, so it always gets its own row and its own `retry_key`, never reusing an older row's key (§18).
- **Exact payload, not just a hash**: `payload_snapshot` holds the literal text that must be sent — a hash alone (the earlier draft of this table's schema) cannot be resent, so this table could not actually recover from a failed delivery without re-deriving content from elsewhere, which is exactly what §18's cron-recovery rule now forbids (below). `payload_hash` exists purely to detect an identity conflict cheaply (§18) — `payload_snapshot` is what a sender actually reads and pushes.
- **`delivery_status = 'superseded'` is the authoritative "do not send this" marker** for `preview_ready` rows only — set when a newer `preview_ready` row for the same session is created (§18's supersession rule). A superseded row is never deleted (it stays as an audit record of what was once pending) and never transitions out of `'superseded'`.
- **Not append-only**: unlike `purchase_capture_session_ingests`/`purchase_capture_lifecycle_events` (§16.6), this table's `delivery_status`/`attempt_count`/`last_attempt_at`/`delivered_at`/`superseded_at`/`last_error`/`updated_at` columns are mutated in place as delivery is attempted, eventually succeeds, or is superseded — the row's identity (`session_id`, `notification_kind`, `notification_version`, `retry_key`) and its `payload_snapshot`/`payload_hash` are what stay immutable, never the whole row.
...
- `finalize_purchase_capture_session(p_session_id, p_expected_generation, p_expected_ingest_revision, p_expected_ingest_hash, p_assembly_status, p_receipt_id_or_null, p_draft_revision_or_null, p_preview_payload_snapshot_or_null, p_fail_reason_or_null)` — mirrors `finalize_physical_inventory_session`; transitions `closing` → `awaiting_confirmation` **only when the caller supplies a real, non-null `receipt_id`/`draft_revision` already written by a preceding `upsert_purchase_receipt_draft` call** (§2.2) — the RPC does not accept a stub/placeholder outcome; a `NULL` receipt_id is only valid on the `→ failed_closed` branch. On the `awaiting_confirmation` branch, in the same transaction, also creates the session's first `preview_ready` outbox row from `p_preview_payload_snapshot` (via the same internal logic `get_or_create_purchase_capture_notification` uses, §16.3) — there is nothing to supersede yet, since this is the document's first preview. This is the RPC that Slice A's schema defines space for but Slice A never calls with an `awaiting_confirmation` target (§21).
- `replace_purchase_capture_draft(p_session_id, p_expected_generation, p_expected_receipt_id, p_expected_draft_revision, p_source_type, p_source_id, p_sender_line_user_id, p_draft_payload, p_preview_payload_snapshot)` — **new (§11.3, §12.2), replaces the earlier two-step `saveDraft()` + `recheck_purchase_capture_draft()` design**. One RPC, one transaction: locks the session **first**, `FOR UPDATE`; refuses `invalid_state` if `status <> 'awaiting_confirmation'` (the "confirmation already won the race" outcome, §12) **before the receipt is touched at all**; refuses `stale_revision` if `receipt_id`/`draft_revision` don't match under the lock; locks the referenced `purchase_receipts` row **second**; applies the full replacement by invoking `0052`'s own `upsert_purchase_receipt_draft` validation/upsert logic internally (no bypass of any `0052` rule); records the resulting new `draft_revision` back onto the already-locked session row; creates the new `preview_ready` outbox row from `p_preview_payload_snapshot`, which — for this kind — also supersedes every older undelivered `preview_ready` row for the same session in the same statement (§16.3, §18); commits all of it together, or none of it. `awaiting_confirmation` → `awaiting_confirmation` only (a self-transition, not a new state). Full step-by-step contract: §12.2.
- `get_or_create_purchase_capture_notification(p_session_id, p_notification_kind, p_notification_version, p_payload_snapshot)` — **new (§18), introduced here in Slice B because `finalize_purchase_capture_session`/`replace_purchase_capture_draft` are its first two callers**; server-computes `payload_hash = sha256(p_payload_snapshot::text)`, then `INSERT ... ON CONFLICT (session_id, notification_kind, notification_version) DO NOTHING RETURNING *`. If the insert lands: for `notification_kind = 'preview_ready'`, also runs `UPDATE purchase_capture_notifications SET delivery_status = 'superseded', superseded_at = now() WHERE session_id = $1 AND notification_kind = 'preview_ready' AND notification_version <> $2 AND delivery_status NOT IN ('delivered','superseded')` in the same statement/transaction (§18) — no separate RPC call, no separate transaction, so a newer preview's existence and its predecessors' supersession are never observably out of sync. If the insert instead conflicts (row already exists): compare the existing row's `payload_hash` to the freshly computed one — **equal → return the existing row unchanged** (idempotent retry of identical content); **different → refuse `notification_identity_conflict` and make no change** (§18) — this table's `payload_snapshot`/`payload_hash` are never overwritten by a conflicting call. Returns the row's `id`, immutable `retry_key`, current `delivery_status`, and `payload_snapshot`, so a caller can both decide whether to skip sending (`delivery_status IN ('delivered','superseded')`) and, on a genuine retry, read back the exact content to (re)send without needing to reconstruct it from anywhere else.
- `record_purchase_capture_notification_attempt(p_notification_id, p_error_or_null)` — **new (§18)**; increments `attempt_count`, sets `last_attempt_at = now()`, and `last_error` if an error was passed — called on every send attempt regardless of outcome, so the row's recovery-eligibility is always current.
- `mark_purchase_capture_notification_delivered(p_notification_id)` — **new (§18)**; sets `delivery_status = 'delivered'`, `delivered_at = now()` — called on a genuine LINE success **and** on a 409 that reuses the row's own `retry_key` (§18), since LINE's 409 for a known retry key means the message was already accepted. Refuses (no-op, or a distinct error) if the row's current `delivery_status` is already `'superseded'` — a superseded row cannot retroactively become delivered.
...
- `mark_purchase_capture_session_posted(p_session_id, p_expected_generation, p_movement_id)` — `confirming` → `posted` (terminal), records `movement_id`. The caller separately calls Slice B's `get_or_create_purchase_capture_notification(session_id, 'posted_success', movement_id, payload_snapshot)` (§12 step 9) — `posted_success` is never subject to supersession (§18), so no special handling is needed here beyond reusing the existing general-purpose RPCs as-is.
...
- **The notification outbox fails closed on a content mismatch.** `get_or_create_purchase_capture_notification` (§16.3, §16.4, §18) never silently overwrites a stored `payload_snapshot`/`payload_hash` — if a caller presents the same `(session_id, notification_kind, notification_version)` with genuinely different content than what is already stored, the call is refused (`notification_identity_conflict`), not merged or replaced. This protects the outbox's own core guarantee (§18): a `retry_key` always corresponds to exactly one piece of content for its entire lifetime.
...
**PROPOSED, reusing existing conventions where they still fit (§2.4, §2.5) but replacing the earlier session-level retry-key columns with a durable notification outbox, `purchase_capture_notifications` (§16.3), because a session-level key cannot safely represent more than one version of the same kind of message.**

- **Webhook reply vs. push**: while still inside the original webhook HTTP request/response cycle (i.e., the close event that triggers immediate finalize, or the confirm event that triggers immediate posting, both complete within the request), use LINE's `reply` API exactly as the current handlers do. A reply send does **not consume the row's `retry_key`** — LINE's reply API has no redelivery-collision problem the way push does — but it still uses the outbox row: the row is created transactionally alongside the content that produced it (§16.3, §16.4), and the sender re-reads the row's live `delivery_status` immediately before sending, whether sending by reply or by push (below). Once work has been deferred past the response (background finalize via `after()`, or a cron-sweep-driven recovery, §8.2/§12.1), push with the row's `retry_key`.
- **Every logical notification is identified by `(session_id, notification_kind, notification_version)`, not by session alone.** A single session can produce many distinct pieces of outbound content over its lifetime: a `preview_ready` notification **per `draft_revision`** (the original finalize preview, and a new one every time `ตรวจใบซื้อใหม่` produces a new revision — each is genuinely different content, not a retry of the same message), one `posted_success` notification identified by `movement_id`, and one `stuck_escalation` notification per escalation. Treating "the preview for revision 3" and "the preview for revision 4" as the same logical notification (as an earlier draft of this document did, by storing one `preview_retry_key` per session) is exactly the bug this section closes: LINE could 409 the updated preview because it reused a key already spent on the stale one.
  - `get_or_create_purchase_capture_notification(session_id, notification_kind, notification_version, payload_snapshot)` (§16.3, §16.4) is called before every send attempt, and its **payload is the exact, immutable content to send** — not merely a hash the caller must somehow reconstruct content from. It is an idempotent get-or-insert on `UNIQUE(session_id, notification_kind, notification_version)`: the first call for a given triple creates the row, its `retry_key`, and its `payload_snapshot`/`payload_hash`; every later call for the *same* triple with **byte-identical** content returns the *same* row unchanged; a later call for the same triple with **different** content is refused `notification_identity_conflict` (§16.4, §17) rather than silently overwriting what was already committed — a caller can never accidentally rewrite history under an existing `retry_key`.
  - **A new `draft_revision` is a new `notification_version`, which is a new row, which is a new `retry_key`.** The old `preview_ready` row for the superseded revision is left exactly as it was — its key is never reused for the new content, so the old key cannot suppress delivery of the new preview via a stale 409.
  - **Different notification kinds never share a row or a key** — `preview_ready` and `posted_success` for the same session always live in different rows with different `retry_key`s, even if both need delivering around the same time.
  - `delivery_status` is checked before sending: if a row is already `'delivered'` or `'superseded'` (below), the caller skips sending entirely — this makes the cron sweep (below) safe to run repeatedly without re-pushing settled or obsolete notifications.
- **Obsolete previews are superseded in the database, not merely "outraced."** Creating a new `preview_ready` row for revision R2 atomically marks every *other* `preview_ready` row for the same session that is still `'pending'`/`'failed'` as `'superseded'`, in the same statement that inserts the R2 row (§16.4) — this is the authoritative model (`delivery_status` gains a `'superseded'` value, §16.3), not a convention the application is trusted to honor.
  - **Delivered historical previews are untouched** — a row already `'delivered'` never transitions to `'superseded'`; it remains a permanent, accurate audit record of what was actually sent, at the revision it was actually sent for.
  - **Cron only ever selects `delivery_status IN ('pending','failed')` rows.** A `'superseded'` row is never a delivery candidate again, for any reason.
  - **A late worker that already loaded an old preview row before it was superseded must re-check the row's live `delivery_status` immediately before its own send attempt** (a fresh read, or an idempotent re-call to `get_or_create_purchase_capture_notification` with the same identity, which returns the row's current state) and abort if it now reads `'superseded'` or `'delivered'` — the row it loaded earlier is a snapshot of a moment in time, not a license to send regardless of what has happened since.
  - **`posted_success` and `stuck_escalation` are never superseded by a preview change** — the supersession UPDATE in `get_or_create_purchase_capture_notification` only ever targets rows where `notification_kind = 'preview_ready'` for the same session; a `posted_success` row's terminal, one-shot nature (§12) means it was never a candidate for supersession in the first place.
  - **An updated preview cannot be sent before its own outbox row commits** — the row (and therefore the supersession of its predecessors) is created inside the *same* transaction as the draft replacement that produced it (§12.2, §16.4), so there is no window where a sender could act on the new content without the database already reflecting it, and no window where a stale row could still read as `'pending'` after its replacement exists.
- **Key existence is not delivery status.** A row existing (and therefore a `retry_key` existing) only means "this content has been decided upon," not "LINE has it." Only `mark_purchase_capture_notification_delivered` (§16.4) — called on a genuine LINE success, or on a 409 that reuses this row's own `retry_key` (meaning LINE already accepted an earlier attempt of this exact content) — sets `delivery_status = 'delivered'`. Every attempt, successful or not, calls `record_purchase_capture_notification_attempt` (§16.4) to bump `attempt_count`/`last_attempt_at`/`last_error`, so a failed attempt remains durable and eligible for the next recovery pass with the *same* `retry_key`, reading the *same* stored `payload_snapshot` — never a freshly re-derived one.
...
- **Behavior if DB succeeds but LINE delivery fails**: the DB-side state (`posted`, `movement_id` set) is already correct and terminal; only the notification is missing, and that fact is durable and queryable — `SELECT * FROM purchase_capture_notifications WHERE delivery_status IN ('pending','failed')`. The cron sweep's "recent terminal sessions with an undelivered close message" pattern (`finalizer.ts:319-334`, §2.4) generalizes directly: select eligible outbox rows (regardless of how old the session is — durable, not time-boxed to 24h the way Physical Inventory's redelivery check is, since a row's identity never expires), re-verify each is still `'pending'`/`'failed'` immediately before sending (above), read the row's own `payload_snapshot` (never re-rendered from the current, possibly-newer, mutable draft state — that is exactly the reconstruction this design forbids), and retry with the row's own `retry_key`.
- **Scheduler recovery**: same cron route proposed in §8.2/§12.1 covers this — one sweep, multiple responsibilities (finalize-due, resume-posting, redeliver-outbox-rows), mirroring how the existing Physical Inventory sweep already does more than one job per pass (`finalizer.ts:336-394`). The sweep never needs to guess which key to use — each undelivered row already carries its own.
- **Duplicate notification prevention**: LINE's own `X-Line-Retry-Key` 409 semantics (§2.5, reused unmodified) — a redelivery attempt with a row's own `retry_key` either succeeds once or 409s harmlessly (both outcomes mark the row delivered); a different notification (different kind or version) always has its own row and key, so it can never collide with or be mistaken for a retry of this one.
...
- Retry of the same preview revision → `get_or_create_purchase_capture_notification` returns the same row/`retry_key`/`payload_snapshot`, exactly one delivered message (§18).
- `ตรวจใบซื้อใหม่` creates a new draft revision → a new `preview_ready` row with a new `notification_version` and a new `retry_key`; the old row's `payload_snapshot`/`retry_key` are untouched (§18).
- The old preview's `retry_key` cannot suppress delivery of the updated preview — sending the new row's content with the new row's key succeeds independently of the old row's delivery state (§18).
- `preview_ready` and `posted_success` notifications for the same session always resolve to different rows and different `retry_key`s (§18).
- A successful (HTTP 200) LINE response marks the notification row `delivered` (§18).
- A duplicate LINE 409 response, using the row's own `retry_key`, also marks the row `delivered` (§18).
- Cron never re-sends a row already marked `delivered` (§18).
- A failed delivery attempt records the attempt (`attempt_count`/`last_attempt_at`/`last_error`) but stays `pending`, remaining eligible for the next recovery pass with the same `retry_key` and the same stored `payload_snapshot` (§18).
- **Same identity, same payload** — two `get_or_create_purchase_capture_notification` calls with identical `(session_id, notification_kind, notification_version)` and byte-identical `payload_snapshot` → same row, same `retry_key`, no error (§16.4, §18).
- **Same identity, different payload fails closed** — a second call with the same identity but a different `payload_snapshot` → refused `notification_identity_conflict`; the originally-stored `payload_snapshot`/`payload_hash` are unchanged (§16.4, §18).
- **Retry uses the stored payload, not a re-render** — after the underlying receipt has moved to a later `draft_revision` (a newer `preview_ready` row now exists), a recovery pass for the *older*, still-undelivered row (before it is marked superseded, or a `posted_success` row after the receipt state has otherwise changed) sends exactly the `payload_snapshot` stored on that row — never a freshly re-rendered message derived from the receipt's current, later state (§18).
- **Thai Unicode payload survives storage/readback exactly** — a `payload_snapshot` containing full Thai text (including combining marks/tone marks) round-trips through insert and read-back with zero byte difference; `payload_hash` recomputed from the read-back value matches the stored `payload_hash` exactly.
- **R1 preview fails delivery** — create a `preview_ready` row for revision R1, simulate a failed push attempt (`record_purchase_capture_notification_attempt` with an error) → row stays `'pending'`/`'failed'`, not `'delivered'`.
- **Recheck creates R2 preview and supersedes R1** — with R1 still undelivered, run `ตรวจใบซื้อใหม่` (`replace_purchase_capture_draft`) to revision R2 → a new `preview_ready` row exists for R2, and R1's row now reads `delivery_status = 'superseded'`, `superseded_at` set.
- **Cron sends R2 only** — run the recovery sweep against a session with both an R1 (`'superseded'`) and an R2 (`'pending'`) `preview_ready` row → exactly one send attempt occurs, against R2; R1 is never selected.
- **A worker that loaded R1 before supersession cannot send it afterward** — simulate a worker that read R1's row, then a concurrent recheck supersedes it, then the worker attempts its send → the worker's immediate pre-send re-check observes `'superseded'` and aborts without calling LINE.
- **Delivered R1 remains in history but is never resent** — if R1 was already marked `'delivered'` before R2's revision was created, assert R1's row stays `'delivered'` (never flipped to `'superseded'`) and is never selected by any later cron pass.
- **`posted_success` remains unaffected by preview supersession** — create a `preview_ready` row and a `posted_success` row for the same session, then trigger another `ตรวจใบซื้อใหม่`/new preview revision → assert the `posted_success` row's `delivery_status` is completely untouched by the preview's supersession UPDATE (§16.4's supersession clause only ever targets `notification_kind = 'preview_ready'`).
...
- **Files/components**: new `supabase/migrations/<timestamp>_purchase_capture_draft_finalization.sql` adding: `finalize_purchase_capture_session` (extended to also create the first `preview_ready` outbox row transactionally, §16.4); `replace_purchase_capture_draft` (§12.2, §16.4 — the sole entry point for `ตรวจใบซื้อใหม่` content, locking the session first, `FOR UPDATE`, refusing `invalid_state` before the receipt is ever touched if the session is no longer `awaiting_confirmation`, then applying the full replacement and the new `preview_ready` row in the same transaction — the lock-ordering discipline the confirm-gate in Slice C depends on, §12); both `purchase_intake_product_registry`/`purchase_intake_unit_alias_registry` tables (§10.3, §16.2); the **`purchase_capture_notifications` outbox table and its three general-purpose RPCs** — `get_or_create_purchase_capture_notification` (with its identity-conflict fail-closed behavior and its `preview_ready`-scoped supersession logic), `record_purchase_capture_notification_attempt`, `mark_purchase_capture_notification_delivered` (§16.3, §16.4, §18) — **introduced here because this slice is the first to produce `preview_ready` content, not deferred to Slice C**; new `src/lib/purchase-capture/parser-adapter.ts` (§9, including the `(line_timestamp_ms, line_event_id)` chunk-ordering step and the identity-resolution step); new `src/lib/purchase-capture/preview.ts` (§11, rendering both the human-readable Thai text and the `payload_snapshot` passed to the outbox RPCs); wiring the cron route (from Slice A) so an eligible `closing` session now actually calls the adapter → resolver → `finalize_purchase_capture_session` with a **real, non-null** `receipt_id`/`draft_revision`/`preview_payload_snapshot`, and so the outbox-redelivery half of §18's recovery sweep can already retry undelivered `preview_ready` rows even before Slice C exists (there is simply nothing yet to escalate to `posted_success`/`stuck_escalation`).
...
7. Every distinct piece of outbound content — each preview revision, the posted confirmation, any stuck escalation — gets its own durable `purchase_capture_notifications` row, its own immutable `retry_key`, and its own immutable, exact `payload_snapshot` (§16.3, §18) — no session-level key is ever reused across changed content, a same-identity/different-content call fails closed (`notification_identity_conflict`) rather than overwriting stored content, an obsolete undelivered preview is database-marked `'superseded'` the moment a newer one exists and is never sent by any sender that re-checks its status first, delivered rows remain immutable audit history, and cron recovery only ever re-sends rows still `'pending'`/`'failed'`, using each row's own stored payload rather than re-deriving one from the current, possibly-newer draft state (§19.3's outbox tests pass in full).
