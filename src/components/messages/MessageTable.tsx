  ...
  if (events.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-slate-400">
        ...
        <p className="text-xs mt-1">ข้อความจาก LINE จะแสดงที่นี่เมื่อมีการส่งเข้ามา</p>
      </div>
    );
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 bg-slate-50 text-left">
            <th className="px-4 py-3 font-medium text-slate-600 whitespace-nowrap">ประเภท</th>
            <th className="px-4 py-3 font-medium text-slate-600 whitespace-nowrap hidden sm:table-cell">ข้อความ</th>
            <th className="px-4 py-3 font-medium text-slate-600 whitespace-nowrap">สถานะ</th>
            <th className="px-4 py-3 font-medium text-slate-600 whitespace-nowrap hidden lg:table-cell">เวลา</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100">
          {events.map((msg) => (
            <tr key={msg.id} className="hover:bg-slate-50 transition-colors">
              <td className="px-4 py-3.5">{messageTypeBadge(msg.message_type)}</td>
              <td className="px-4 py-3.5 hidden sm:table-cell">
                <span className="text-slate-600 truncate max-w-xs block">
                  {msg.raw_text ?? <span className="text-slate-300 italic">—</span>}
                </span>
              </td>
              <td className="px-4 py-3.5">{statusBadge(msg.is_processed)}</td>
