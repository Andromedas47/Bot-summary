  ...
  if (events.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-slate-400">
        ...
        <p className="text-xs mt-1">Messages will appear here when your LINE webhook receives events.</p>
      </div>
    );
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 bg-slate-50 text-left">
            <th className="px-4 py-3 font-medium text-slate-600 whitespace-nowrap">Event type</th>
            <th className="px-4 py-3 font-medium text-slate-600 whitespace-nowrap">Message type</th>
            <th className="px-4 py-3 font-medium text-slate-600 whitespace-nowrap">Source</th>
            <th className="px-4 py-3 font-medium text-slate-600 whitespace-nowrap hidden md:table-cell">Source ID</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {events.map((event) => (
            <tr key={event.id} className="hover:bg-slate-50 transition-colors">
              ...
              <td className="px-4 py-3">{messageTypeBadge(event.message_type)}</td>
              <td className="px-4 py-3">
                <Badge variant="default">{event.source_type}</Badge>
              </td>
