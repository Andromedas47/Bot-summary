...
- **`ตรวจใบซื้อใหม่`** ("recheck the purchase") is the operator's path forward after fixing a registry gap (§10.3). It calls `recheck_purchase_capture_draft` (§16.4): re-reads the session's original, unchanged durable ingest set (§8), re-runs the unchanged pure parser (§9.1 steps 1-4) and identity resolution (§9.1 step 5) against the *current* registry state, performs another full-replace `saveDraft` (§2.2) under the *same* `documentNamespace`/`documentKey` (§6.3 — this is still the same document, not a new one), then calls `recheck_purchase_capture_draft` to record the resulting new `draft_revision` on the session — this call locks the session row first (the same lock-ordering discipline as `begin_purchase_capture_confirmation`, §12) and refuses if the session is no longer `awaiting_confirmation` (i.e. a concurrent `ยืนยันซื้อ` already won the race, §12) — and sends a fresh preview (§11.1 or §11.2, depending on whether blockers remain). It never calls `confirm()` — recheck only ever produces a new draft, never a confirmation, even if every blocker happens to clear.
...
3. beginResult = begin_purchase_capture_confirmation({ sessionId,
   ...
   a. Lock the session row FOR UPDATE — session locked before receipt,
      always; `recheck_purchase_capture_draft` (§11.3) uses the identical
...
| **Race: `ตรวจใบซื้อใหม่` commits its new `draft_revision` while a concurrent `ยืนยันซื้อ` is in flight against the old one** | `begin_purchase_capture_confirmation` locks the session first and re-checks `receipt_id`/`draft_revision` under that lock (§12 step 3d) | If recheck's commit lands first: confirm's RPC call sees the mismatch and refuses `stale_revision` — session stays `awaiting_confirmation`, no partial transition, no confirm() call. The stale confirm attempt must be retried by the caller against the new preview |
| **Race: `ยืนยันซื้อ` transitions the session to `confirming` while a concurrent `ตรวจใบซื้อใหม่` is in flight** | `recheck_purchase_capture_draft` locks the session first and re-checks `status = 'awaiting_confirmation'` under that lock (§11.3) | If confirm's commit lands first: recheck's RPC call sees `status = 'confirming'` and refuses `invalid_state` — the new draft is never attached to a session that has already moved on. Both RPCs lock the session before anything else, in the same order, so this is a clean win/lose outcome, never a deadlock |
...
| LINE reply/push failure | `pushLineMessage`'s existing 409/retry-key handling (§2.5, reused unmodified); the notification row's `delivery_status` stays non-`delivered` (§18) | Cron selects the durable undelivered `purchase_capture_notifications` row and retries with that row's own `retry_key` — the same key every time, because it never changes once the row exists; a different logical notification (different `notification_kind`/`notification_version`) always has its own row and its own key, so it can never be treated as a retry of this one |
| A new preview supersedes an undelivered old one (`ตรวจใบซื้อใหม่` runs before the previous preview push succeeded) | The old `preview_ready` row (keyed on the old `draft_revision`) stays exactly as it was; a *new* row is created for the new `draft_revision` (§18) | The old row's key is never reused for the new content, so LINE cannot 409-suppress the updated preview; the old row simply stops mattering once staff see and act on the newer one — no cleanup RPC is required for V1 |
...
- `20260805020000_purchase_capture_confirm_post.sql` (Slice C — `purchase_capture_notifications` table, `begin_purchase_capture_confirmation`, `mark_purchase_capture_session_posted`, and the three notification-outbox RPCs, §21)
...
### 16.3 Table (Slice C) — durable notification outbox

**PROPOSED**, replacing the earlier per-session retry-key columns (§18):

- **`purchase_capture_notifications`** — `id` (uuid pk), `session_id` (FK → `purchase_capture_sessions`), `notification_kind` (CHECK IN `'preview_ready','posted_success','stuck_escalation'`), `notification_version` (text — the exact `draft_revision` for `preview_ready`, `movement_id` for `posted_success`, one fixed escalation identifier for `stuck_escalation`), `retry_key` (uuid, `DEFAULT gen_random_uuid()`, immutable once set), `payload_hash` (text — a hash/snapshot of the exact content sent, so a row can be audited without needing to re-derive the message), `delivery_status` (CHECK IN `'pending','delivered','failed'`, `DEFAULT 'pending'`), `attempt_count` (`DEFAULT 0`), `last_attempt_at`, `delivered_at`, `last_error`, `created_at`, `updated_at`.
...
- **Not append-only**: unlike `purchase_capture_session_ingests`/`purchase_capture_lifecycle_events` (§16.6), this table's `delivery_status`/`attempt_count`/`last_attempt_at`/`delivered_at`/`last_error`/`updated_at` columns are mutated in place as delivery is attempted and eventually succeeds — the row's identity (`session_id`, `notification_kind`, `notification_version`, `retry_key`) is what stays immutable, not the whole row.
...
- `recheck_purchase_capture_draft(p_session_id, p_expected_generation, p_receipt_id, p_draft_revision)` — **new (§11.3), locking discipline hardened for the confirm race (§12)**; locks the session row **first**, `FOR UPDATE`; refuses `invalid_state` if `status <> 'awaiting_confirmation'` (this is the "confirmation already won the race" outcome, §12); only if still `awaiting_confirmation`, re-records `receipt_id` (unchanged — same document) and the new `draft_revision` after the caller has already re-run the resolver + parser + `saveDraft` against the session's original, untouched ingest set. `awaiting_confirmation` → `awaiting_confirmation` only (a self-transition, not a new state).

**Slice C** — the confirm-gate, posting, and the notification outbox:

- `begin_purchase_capture_confirmation(p_session_id, p_expected_generation, p_expected_receipt_id, p_expected_draft_revision, p_source_type, p_source_id, p_sender_line_user_id)` — **new, replaces the earlier `transition_purchase_capture_session_confirming` (§12 step 3)**. The **sole authoritative confirm-gate**, not an application-trusted transition: in one transaction, locks the session row `FOR UPDATE` (before the receipt — same order `recheck_purchase_capture_draft` uses, so the two can never deadlock, §12), re-checks ownership/generation/status, re-checks `receipt_id`/`draft_revision` match under that lock, locks the referenced `purchase_receipts` row `FOR UPDATE`, re-checks its `status`/`draft_revision`, evaluates blocking blockers directly from the locked `purchase_receipt_items` rows per §10.3, and only transitions `awaiting_confirmation` → `confirming` if every check passes; otherwise refuses with a specific reason (`ownership_mismatch`, `generation_mismatch`, `invalid_state`, `stale_revision`, `receipt_not_draft`, `blocking_blocker_present`) and makes no change. Unlike the earlier design, **no RPC in this surface trusts an application-side blocker check** — every gate that matters is re-evaluated inside the RPC itself, under lock, against the authoritative row state.
...
- `get_or_create_purchase_capture_notification(p_session_id, p_notification_kind, p_notification_version, p_payload_hash)` — **new (§18)**; `INSERT ... ON CONFLICT (session_id, notification_kind, notification_version) DO UPDATE SET updated_at = now() RETURNING *` — idempotent get-or-insert. Returns the row's `id`, immutable `retry_key`, and current `delivery_status`/`attempt_count`, so the caller can skip sending entirely if `delivery_status = 'delivered'` already.
- `record_purchase_capture_notification_attempt(p_notification_id, p_error_or_null)` — **new (§18)**; increments `attempt_count`, sets `last_attempt_at = now()`, and `last_error` if an error was passed — called on every push attempt regardless of outcome, so the row's recovery-eligibility is always current.
- `mark_purchase_capture_notification_delivered(p_notification_id)` — **new (§18)**; sets `delivery_status = 'delivered'`, `delivered_at = now()` — called on a genuine LINE success **and** on a 409 that reuses the row's own `retry_key` (§18), since LINE's 409 for a known retry key means the message was already accepted.

### 16.5 RLS posture

RLS **enabled** on all six new tables (three from Slice A, two registries from Slice B, one notification outbox from Slice C), **no policies defined** — identical to `0047`'s posture (§2.4) — access exists only through the `SECURITY DEFINER` RPCs above, exactly matching the established convention.
...
- **The confirm-gate re-evaluates its own preconditions; it does not trust the caller.** `begin_purchase_capture_confirmation` (§12, §16.4) is the one place in this design where a wrong caller-side decision (an app-level blocker check that ran against a now-stale draft) would have real consequences — a receipt with an `UNRESOLVED` item permanently confirmed. Accordingly it is the one RPC in this surface explicitly designed to distrust its caller: revision, ownership, and blocking-blocker state are all re-read from the locked row, not accepted as parameters to trust. No other RPC in §16.4 needs this property, because none of the others gate an irreversible transition the way this one does.
...
- **Webhook reply vs. push**: while still inside the original webhook HTTP request/response cycle (i.e., the close event that triggers immediate finalize, or the confirm event that triggers immediate posting, both complete within the request), use LINE's `reply` API exactly as the current handlers do. Replies **never** need an outbox row or a retry key — LINE's reply API has no redelivery-collision problem the way push does, so the outbox exists purely for the push case. Once work has been deferred past the response (background finalize via `after()`, or a cron-sweep-driven recovery, §8.2/§12.1), create/use the appropriate outbox row and push with its `retry_key`, below.
- **Every logical notification is identified by `(session_id, notification_kind, notification_version)`, not by session alone.** A single session can produce many distinct pieces of outbound content over its lifetime: a `preview_ready` notification **per `draft_revision`** (the original finalize preview, and a new one every time `ตรวจใบซื้อใหม่` produces a new revision — each is genuinely different content, not a retry of the same message), one `posted_success` notification identified by `movement_id`, and one `stuck_escalation` notification per escalation. Treating "the preview for revision 3" and "the preview for revision 4" as the same logical notification (as an earlier draft of this document did, by storing one `preview_retry_key` per session) is exactly the bug this section closes: LINE could 409 the updated preview because it reused a key already spent on the stale one.
  - `get_or_create_purchase_capture_notification(session_id, notification_kind, notification_version, payload_hash)` (§16.4) is called before every send attempt. It is an idempotent get-or-insert on `UNIQUE(session_id, notification_kind, notification_version)` — the first call for a given triple creates the row and its `retry_key`; every later call for the *same* triple (a retry of that exact content) returns the *same* row, unchanged.
...
- **Behavior if DB succeeds but LINE delivery fails**: the DB-side state (`posted`, `movement_id` set) is already correct and terminal; only the notification is missing, and that fact is durable and queryable — `SELECT * FROM purchase_capture_notifications WHERE delivery_status <> 'delivered'`. The cron sweep's "recent terminal sessions with an undelivered close message" pattern (`finalizer.ts:319-334`, §2.4) generalizes directly: select undelivered outbox rows (regardless of how old the session is — durable, not time-boxed to 24h the way Physical Inventory's redelivery check is, since a row's identity never expires) and retry each with its own `retry_key`.
...
- **Confirm-vs-recheck race, recheck wins first**: interleave `recheck_purchase_capture_draft` (committing a new `draft_revision`) ahead of a concurrent `begin_purchase_capture_confirmation` call built against the old revision → the confirm call refuses `stale_revision`, session remains `awaiting_confirmation`, no transition to `confirming` occurred (§12).
- **Confirm-vs-recheck race, confirmation wins first**: interleave `begin_purchase_capture_confirmation` (committing the transition to `confirming`) ahead of a concurrent `recheck_purchase_capture_draft` call → the recheck call refuses `invalid_state`, the new draft it computed is never attached to the (now `confirming`) session (§12).
- **No deadlock under concurrent confirm + recheck**: both RPCs lock the session row before the receipt row, in the same order (§12) — run many concurrent pairs against the same session in a stress test and assert every call either succeeds or refuses cleanly, never blocks indefinitely or raises a Postgres deadlock error.
- **No path leaves a stale draft in `confirming`**: after any confirm-vs-recheck interleaving, assert that whichever draft revision the session ends up `confirming` against (if any) is exactly the revision `begin_purchase_capture_confirmation` itself locked and verified — never a revision superseded by a recheck that committed first.
...
- Retry of the same preview revision → `get_or_create_purchase_capture_notification` returns the same row/`retry_key`, exactly one delivered message (§18).
- `ตรวจใบซื้อใหม่` creates a new draft revision → a new `preview_ready` row with a new `notification_version` and a new `retry_key`; the old row and its key are untouched (§18).
...
- A failed delivery attempt records the attempt (`attempt_count`/`last_attempt_at`/`last_error`) but stays `pending`, remaining eligible for the next recovery pass with the same `retry_key` (§18).
...
- **Files/components**: new `supabase/migrations/<timestamp>_purchase_capture_draft_finalization.sql` adding `finalize_purchase_capture_session` and `recheck_purchase_capture_draft` (§16.4 — the latter locks the session row first, `FOR UPDATE`, and refuses `invalid_state` if the session is no longer `awaiting_confirmation`, the lock-ordering discipline the confirm-gate in Slice C depends on, §12) plus both `purchase_intake_product_registry`/`purchase_intake_unit_alias_registry` tables (§10.3, §16.2); new `src/lib/purchase-capture/parser-adapter.ts` (§9, including the `(line_timestamp_ms, line_event_id)` chunk-ordering step and the identity-resolution step); new `src/lib/purchase-capture/preview.ts` (§11); wiring the cron route (from Slice A) so an eligible `closing` session now actually calls the adapter → resolver → `PurchaseReceiptService.saveDraft` → `finalize_purchase_capture_session` with a **real, non-null** `receipt_id`/`draft_revision`.
...
### Slice C — Confirmation gate + notification outbox + idempotent P2C posting + recovery
- **Files/components**: new `supabase/migrations/<timestamp>_purchase_capture_confirm_post.sql` adding the `purchase_capture_notifications` table (§16.3), `begin_purchase_capture_confirmation`/`mark_purchase_capture_session_posted` RPCs and the three notification-outbox RPCs — `get_or_create_purchase_capture_notification`, `record_purchase_capture_notification_attempt`, `mark_purchase_capture_notification_delivered` (§16.4); `src/lib/purchase-capture/confirm-flow.ts` implementing §12's exact sequence, including step 2's **advisory-only** blocker read (used purely to word an early refusal reply; the actual gate is `begin_purchase_capture_confirmation`, called in step 3, which is the only thing permitted to transition the session) and the `ตรวจใบซื้อใหม่`/`ยืนยันซื้อ`/`ยกเลิกซื้อ` LINE command dispatch; extending the Slice A cron route to also perform §12.1's recovery sweep and §18's outbox-driven notification-redelivery sweep (selecting `purchase_capture_notifications` rows with `delivery_status <> 'delivered'`, retrying each with its own row's `retry_key`); the single new call site into `webhook-service.ts`'s `processOne` chain (§2.7) — this is the only slice that touches the existing webhook file, and only by adding one call, not modifying existing branches.
- **Migration dependency**: Slices A and B.
- **Tests**: §19.2's confirm-vs-recheck race tests (recheck-wins-first and confirm-wins-first outcomes, no deadlock); §19.3 in full (posting-specific, confirm-gate, and outbox-delivery cases); the crash-recovery case exercised explicitly (kill the process between confirm and post in a test harness, then run the recovery sweep).
- **Definition of Done**: the full journey in the task's stated goal works end-to-end against a disposable local Postgres, including: a blocked draft correctly refusing `ยืนยันซื้อ` via `begin_purchase_capture_confirmation`'s own authoritative check (not the app-side read); `ตรวจใบซื้อใหม่` clearing the block; the confirm-vs-recheck race resolving deterministically in both directions with no deadlock and no stale draft ever reaching `confirming`; a changed preview revision always getting its own notification row/key, never reusing an older one; and the crash-recovery case. `webhook-service.ts`'s existing routing order/behavior for every other feature is unchanged (regression-tested by the existing suite, §2.4's test list).
...
3. A `ตรวจใบซื้อใหม่` racing a `ยืนยันซื้อ` against the same session always resolves deterministically and without deadlock: whichever RPC locks the session row first wins, the other is refused (`stale_revision` or `invalid_state`, §12), and no interleaving ever leaves a stale draft sitting in `confirming` or lets a blocked draft slip through (§19.2's race tests pass in full).
4. After a registry fix, `ตรวจใบซื้อใหม่` re-resolves the original ingest set and produces a new draft revision without requiring the document to be re-sent, and a subsequent `ยืนยันซื้อ` correctly binds to that newest revision (§11.3).
...
7. Every distinct piece of outbound content — each preview revision, the posted confirmation, any stuck escalation — gets its own durable `purchase_capture_notifications` row and its own immutable `retry_key` (§16.3, §18); no session-level key is ever reused across changed content, delivery status is durable and queryable, and cron recovery only ever re-sends rows still `pending` (§19.3's outbox tests pass in full).
...
The durable-session, parser, persistence, and ledger layers this flow needs are either already fully built and Production-verified (P2B parser, P2B persistence, P2C ledger — §2.1-§2.3, §3) or are a direct, low-risk adaptation of a pattern already running successfully in Production for a structurally identical problem (the Physical Inventory close barrier — §2.4, §8, extended with the one-message inline-close path, §8.4). No part of this design requires inventing a new safety mechanism from scratch; every idempotency and ownership guarantee in §13/§17 is either reused verbatim or built by copying an existing, tested shape. Two correctness gaps found in an earlier draft of this design are now closed: the confirm-gate is authoritative and race-safe — `begin_purchase_capture_confirmation` (§12, §16.4) re-verifies ownership, revision, and blocking-blocker state itself, under lock, rather than trusting an application-side check that a concurrent `ตรวจใบซื้อใหม่` could have already made stale — and outbound notifications are identified per logical notification (`session_id`, `notification_kind`, `notification_version`, §16.3, §18), so a new preview revision can never be suppressed by a key already spent on the one it replaced.
